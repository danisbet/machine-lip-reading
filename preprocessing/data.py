...
def load_data(datapath, verbose=False, num_samples=-1, ctc_encoding=False):
    ...
    for root, dirs, files in os.walk(datapath):
        for name in files:
            if '.mpg' in name:
                if verbose is True:
                    print("reading: " + os.path.join(root, name))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Preprocessing GRID dataset for lip reading.')
    parser.add_argument("-sid", dest='speaker_id', default=1, type=int, help="speaker id") 
    parser.add_argument("-n", dest='num_samples', default=-1, type=int, help="number of samples")
    args = parser.parse_args() 
    speaker_id = args.speaker_id
    num_samples = args.num_samples
 
    X, y, le, oh = load_data('/global/scratch/alex_vlissidis/lipreading_data/s' + str(speaker_id), verbose=True, ctc_encoding=False, num_samples=num_samples)
    print("X:", X.shape)
    print("y:", y.shape)
    
    np.savez_compressed('/global/scratch/alex_vlissidis/lipreading_data/s' + str(speaker_id) + '/X', x=X)
    np.savez_compressed('/global/scratch/alex_vlissidis/lipreading_data/s' + str(speaker_id) + '/y', y=y)
    pickle.dump(le, open('/global/scratch/alex_vlissidis/lipreading_data/s' + str(speaker_id) + '/le.p', 'wb+'))
    pickle.dump(oh, open('/global/scratch/alex_vlissidis/lipreading_data/s' + str(speaker_id) + '/oh.p', 'wb+'))
